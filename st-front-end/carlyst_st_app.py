...
def user_input_features(data, X):
    ...
    POWER = st.sidebar.slider('Power', int(X.power.min()), int(X.power.max()), int(round(data.power.mean(),0)), help='CV')
    CO2 = st.sidebar.slider(u'CO\u2082 emissions', int(X.co2_emiss.min()), int(X.co2_emiss.max()), int(round(data.co2_emiss.mean(),0)), help='g/m\u00B3')
    HEIGHT = st.sidebar.slider('Height', int(X.height.min()), int(X.height.max()), int(round(data.height.mean(),0)), help='cm')
    LENGTH = st.sidebar.slider('Length', int(X.length.min()), int(X.length.max()), int(round(data.length.mean(),0)), help='cm')
    WIDTH = st.sidebar.slider('Width', int(X.width.min()), int(X.width.max()), int(round(data.width.mean(),0)), help='cm')
    TRUNK = st.sidebar.slider('Trunk volume', int(X.trunk_vol.min()), int(X.trunk_vol.max()), int(round(data.trunk_vol.mean(),0)), help='L')
    SPEED = st.sidebar.slider('Max speed', int(X.max_speed.min()), int(X.max_speed.max()), int(round(data.max_speed.mean(),0)), help='km/h')
    CONS = st.sidebar.slider('Mixed consumption', float(X.mixed_cons.min()), float(X.mixed_cons.max()), float(data.mixed_cons.mean()), help='L/100km')
    WEIGHT = st.sidebar.slider('Weight', int(X.weight.min()), int(X.weight.max()), int(round(data.weight.mean(),0)), help='kg')
    TANK = st.sidebar.slider('Tank volume', int(X.tank_vol.min()), int(X.tank_vol.max()), int(round(data.tank_vol.mean(),0)), help='L')
    ACC = st.sidebar.slider('Acceleration', float(X.acc.min()), float(X.acc.max()), float(data.acc.mean()), help='s')
...
def main():
    ...
    # Title logo
    st.image("https://drive.google.com/uc?export=view&id=1cf6kuyI7QMg5VDDLlsbfL-Yrpi-OF_ap", width=None, use_column_width='always')  #685
    st.write("""
